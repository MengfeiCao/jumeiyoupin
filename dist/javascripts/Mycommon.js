...
function downCount(dateString) {
  ...
  var now = new Date();
  var Dtime = target.getTime() - now.getTime(); // 获取时间差的时分秒
...
function jsonp(url, jsonp_key) {
  return new Promise(function (resolve, reject) {
    ...
    var script = document.createElement("script"); // 当前url之中是否存在 ? （存在问好表示已经有数据了），我应该用& 去拼接数据，反之则用 ?;

    url = url + (/\?/.test(url) ? "&" : "?") + jsonp_key + "=" + randomName;
    script.src = url; // 3. 标签放入页面之中;

    document.body.appendChild(script); // 4. 清理垃圾;
