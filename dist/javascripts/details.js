...
// 元素选择
var normal_wrap = _(".Magnifier-normal"); // 大图焦点


var big_wrap = _(".Magnifier-big"); // normal图焦点
...
var small_choice = small_wrap.children; // console.log(small_choice);
// -----------------放大镜------------------
// 计算比例
...
big_img.style.height = normal_wrap.offsetHeight * prop + "px"; // 鼠标移入显示
...
function focusMove(evt) {
  var e = evt || window.event; // 边界检测

  var _left = e.offsetX - focus.offsetWidth / 2;

  var _top = e.offsetY - focus.offsetHeight / 2; // 判断最小值


  _left = _left <= 0 ? 0 : _left;
  _top = _top <= 0 ? 0 : _top; // 判断最大值
  ...
  _top = _top >= topMax ? topMax : _top; // 左侧小图移动

  focus.style.left = _left + "px";
  focus.style.top = _top + "px"; // 右侧大图移动
...
small_choice = Array.from(small_choice); // console.log(small_choice);
// 给每个 i 添加点击事件

small_choice.forEach(function (item) {
...
function change(item) {
  ...
  item.className = "active"; // 替换图片
  // 首先先现获取数据

  var bigSrc = item.getAttribute("data-big");
  var normalSrc = item.getAttribute("data-normal"); // console.log(bigSrc,normalSrc);
